...
namespace BusinessLayer.ValidationRules
{
    public class AddressValidatior:AbstractValidator<Address>
    {
        public AddressValidatior() 
        {
            ...
            RuleFor(x => x.Description1).MaximumLength(25).WithMessage("Lütfen açıklamayı kısalat");
            RuleFor(x => x.Description2).MaximumLength(25).WithMessage("Lütfen açıklamayı kısalat");
            RuleFor(x => x.Description3).MaximumLength(25).WithMessage("Lütfen açıklamayı kısalat");
            RuleFor(x => x.Description4).MaximumLength(25).WithMessage("Lütfen açıklamayı kısalat");
        }
    }
}
